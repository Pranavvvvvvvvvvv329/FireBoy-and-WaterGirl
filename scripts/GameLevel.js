...
class GameLevel extends Phaser.Scene {
  ...
  init(data){
   ...
   // To allow repeating of the cycle of levelswithout problems
    /* explantion:
    ...
    */
    this.levelStartTime = Math.floor(new Date().getTime() / 1000);
     ...
     /* this.physics.world.createDebugGraphic();
      ...
      if (this.Data.sleepTrap) {

    this.Data.sleepTrap.forEach((item) => {
    ...
    this.Data.sleepTrap.forEach((item) => {
        ...
        const char1InTrap =
            Math.abs(this.character1.x - item[0]) < 20 &&
            Math.abs(this.character1.y - item[1]) < 20;

        const char2InTrap =
            Math.abs(this.character2.x - item[0]) < 20 &&
            Math.abs(this.character2.y - item[1]) < 20;

        if (!char1InTrap && !char2InTrap) {
            this.sleepTrapTriggered = false;
        }

        if (
            !this.sleeping &&
            !this.sleepTrapTriggered &&
            (char1InTrap || char2InTrap)
        ) {

            this.sleepTrapTriggered = true;

            if (char1InTrap) {
                this.activateSleepTrap(this.character1);
            } else {
                this.activateSleepTrap(this.character2);
            }
        }
...
if (this.Data.sleepTrap) {
    ...
    this.Data.sleepTrap.forEach((item) => {
        ...
        const char1InTrap =
            Math.abs(this.character1.x - item[0]) < 40 &&
            Math.abs(this.character1.y - item[1]) < 40;

        const char2InTrap =
            Math.abs(this.character2.x - item[0]) < 40 &&
            Math.abs(this.character2.y - item[1]) < 40;
...
if (x1 > item[0]-17   &&   x1 <= item[0]+17  &&   y1 > item[1]-8   &&    y1 <= item[1]+8) {
 ...
 this.loseHeart();
    ...
    sleepText.setOrigin(0.5);
    sleepText.setDepth(100);
